...
正體中文 UI。同系統的網頁端已定案以下三組，**App 端沿用同一套**以維持一致：

| 用途 | 字型 | 字重 | 退回順序 |
|---|---|---|---|
| 無襯線（預設） | **Noto Sans TC**（思源黑體 TC） | 400 / 500 / 600 / 700 | PingFang TC → Microsoft JhengHei → system-ui |
| 襯線（視覺重心） | **Noto Serif TC**（思源宋體 TC） | 500 / 600 / 700 | Songti TC → serif |
| 等寬 | **IBM Plex Mono** | 400 / 500 | ui-monospace → monospace |

**用在哪裡**

- **襯線體**只用在需要視覺重心的地方：站點名稱、完成回饋的大標、件數大數字。
  用量要克制，它是重音不是底色。
- **其餘全部**用無襯線。
- **需要對齊的數字**（件數、預計抵達時間、座標、時間戳）一律用**等寬數字**。

> ⚠️ **IBM Plex Mono 沒有中文字符。** 它只負責數字與英數代碼，
> 中文會退回 fallback。因此**不要**為了數字整段換成等寬字型 -
> 數字經常跟中文在同一行（例如「第 3 站 / 共 19 站」）。
> 正確做法是保持無襯線字族，只開啟**表格數字**特性
> （CSS 的 `font-variant-numeric: tabular-nums`；
> Flutter 的 `FontFeature.tabularFigures()`）。

**App 端有一個關鍵差異：字型必須打包進 App，不可執行期下載。**

網頁端是從字型 CDN 載入（Google Fonts 以 unicode-range 切分 CJK 子集，載入量遠小於整包）。
但這支 App 是**離線優先**的 - 司機在地下停車場、大樓死角照樣要能用。
執行期下載字型代表沒訊號時版面會退回系統字型，
在點交這種需要看清楚數字的畫面上不可接受。

因此思源字型要以資產形式打包。代價是安裝檔會明顯變大，
所以需要做**字集裁切**（保留常用漢字、標點、數字、拉丁字母即可，
不需要整包 CJK）。這件事屬於工程實作，設計端不必處理，
但**設計稿請以「字型一定在」為前提**，不要為了字型可能載不到而做替代版面。
